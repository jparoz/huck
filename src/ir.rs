...
pub enum Expression {
    ...
    /// An occurence of an anonymous name,
    /// generated by the compiler.
    /// See [`Pattern::BindInternal`].
    ReferenceInternal(usize),
...
pub enum Pattern {
    ...
    /// Bind a new anonymous variable,
    /// invisible to the Huck user (i.e. generated by the compiler).
    /// See [`Definition::from`] for information on how this is used.
    BindInternal(usize),
...
impl From<(Name, Vec<ast::Assignment<Name>>)> for Definition {
    fn from((name, mut assignments): (Name, Vec<ast::Assignment<Name>>)) -> Self {
        ...
        // and is impossibly by construction for let bindings.
        ...
        let rhs = if assignments.len() == 1 {
            ...
            let args: Vec<Pattern> = (0..num_args).map(Pattern::BindInternal).collect();
            let case_expr = Box::new(Expression::Tuple(
                (0..num_args).map(Expression::ReferenceInternal).collect(),
            ));
